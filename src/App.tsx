...
import { Search, MapPin, Star, Award, Users, MessageCircle, Mail, Download, CheckSquare, Square } from "lucide-react"
...
export default function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true)
        // تحميل البيانات والأسعار معاً
        const [data] = await Promise.all([
          loadBillboardsFromExcel(),
  ...
  }, [searchTerm, selectedMunicipalities, selectedCities, selectedAreas, selectedSizes, selectedAvailability, billboards, showAllBillboards, currentPage, itemsPerPage])
...
مع تحيات موقع الفارس الذهبي للدعاية والإعلان
        ...
        <table>
          <thead>
            <tr>
              ...
              ${includeImages ? '<th style="width: 8%;">صورة</th>' : ''}
              <th style="width: ${includeImages ? (pricing?.includePricing ? '14%' : '20%') : (pricing?.includePricing ? '20%' : '28%')};">الموقع</th>
              <th style="width: 8%;">البلدية</th>
              <th style="width: 8%;">المنطقة</th>
              <th style="width: 6%;">المقاس</th>
              <th style="width: 5%;">الأوجه</th>
              <th style="width: 6%;">النوع</th>
              ${pricing?.includePricing ? `
              <th style="width: 5%;">الفئة</th>
              <th style="width: 8%;">السعر</th>
              <th style="width: 8%;">بعد الخصم</th>
              ` : `
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      {/* Dynamic Background with Fractal Glass Effect */}
      <DynamicBackground />

      <Header theme={theme} toggleTheme={toggleTheme} totalBillboards={billboards.filter(b => b.status === "متاح").length} />

      <section className="relative z-10 py-16 overflow-hidden page-transition">
        <div 
          className="absolute inset-0 bg-gradient-to-r from-primary/10 via-transparent to-primary/10 parallax-layer" 
          style={{ transform: `translateY(${scrollY * 0.05}px)` }}
        />
        <div className="container mx-auto px-4 text-center relative">
          <h2 
            className="text-4xl md:text-5xl font-black mb-6 tracking-tight leading-tight text-foreground fade-slide-up"
            style={{ transform: `translateY(${scrollY * -0.1}px)` }}
          >
            الرائدون في عالم الدعاية والإعلان
          </h2>
          <p 
            className="text-xl md:text-2xl mb-8 max-w-4xl mx-auto leading-relaxed font-bold text-muted-foreground fade-slide-up stagger-1"
            style={{ transform: `translateY(${scrollY * -0.05}px)` }}
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mt-8 md:mt-12">
            <div className="glass gold-border-glow rounded-xl md:rounded-2xl p-4 md:p-6 card-hover fade-slide-up stagger-1 hover-float">
              <Award className="w-8 h-8 md:w-12 md:h-12 mx-auto mb-2 md:mb-4 text-primary" />
              <h3 className="text-xl md:text-3xl font-black mb-1 md:mb-2 text-foreground">+20</h3>
              <p className="text-sm md:text-lg font-bold text-muted-foreground">سنة خبرة</p>
            </div>
            <div className="glass gold-border-glow rounded-xl md:rounded-2xl p-4 md:p-6 card-hover fade-slide-up stagger-2 hover-float">
              <Users className="w-8 h-8 md:w-12 md:h-12 mx-auto mb-2 md:mb-4 text-primary" />
              <h3 className="text-xl md:text-3xl font-black mb-1 md:mb-2 text-foreground">+800</h3>
              <p className="text-sm md:text-lg font-bold text-muted-foreground">عميل راضي</p>
            </div>
            <div className="glass gold-border-glow rounded-xl md:rounded-2xl p-4 md:p-6 card-hover fade-slide-up stagger-3 hover-float">
              <MapPin className="w-8 h-8 md:w-12 md:h-12 mx-auto mb-2 md:mb-4 text-primary" />
              <h3 className="text-lg md:text-2xl font-black mb-1 md:mb-2 text-foreground">مواقع كافية</h3>
              <p className="text-sm md:text-lg font-bold text-muted-foreground">لحملتك</p>
            </div>
            <div className="glass gold-border-glow rounded-xl md:rounded-2xl p-4 md:p-6 card-hover fade-slide-up stagger-4 hover-float">
              <Star className="w-8 h-8 md:w-12 md:h-12 mx-auto mb-2 md:mb-4 text-primary" />
              <h3 className="text-xl md:text-3xl font-black mb-1 md:mb-2 text-foreground">100%</h3>
              <p className="text-sm md:text-lg font-bold text-muted-foreground">جودة مضمونة</p>
            </div>
          </div>
        </div>
      </section>

      <main className="container mx-auto px-4 py-12 relative z-10 page-transition">
        <SearchFilters
        ...
        {showMap && <InteractiveMap billboards={filteredBillboards} onImageView={setSelectedImage} selectedBillboards={selectedBillboards} onToggleSelection={toggleBillboardSelection} onSelectMultiple={selectMultipleBillboards} onDownloadSelected={handleDownloadSelected} />}
        ...
        {selectedBillboards.size > 0 && (
          <div className="fixed bottom-4 left-2 right-2 md:left-4 md:right-4 md:bottom-20 glass gold-border-glow rounded-xl p-3 md:p-4 z-50">
        ...
        <div className="relative py-12 -mt-4" style={{ marginLeft: 'calc(-50vw + 50%)', marginRight: 'calc(-50vw + 50%)', width: '100vw' }}>
          ...
          {/* Cards Grid */}
          <div
            className={`relative z-10 grid gap-4 md:gap-6 px-4 md:px-8 lg:px-16 xl:px-24 ${viewMode === "grid" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1"}`}
          >
            {loading ? (
              // Skeleton loading state
              Array.from({ length: 8 }).map((_, index) => (
                <BillboardCardSkeleton key={`skeleton-${index}`} />
              ))
            ) : (
