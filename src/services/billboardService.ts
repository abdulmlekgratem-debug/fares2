  ...
  try {
    ...
    const billboards = jsonData
      ...
      .map((billboard: any, index: number) => {
        ...
        const expiryDateRaw = billboard['تاريخ انتهاء الإيجار'] || billboard['تاريخ الانتهاء'] || billboard['تاريخ انتهاء العقد'] || ''
        let status = 'متاح'
        let expiryDate: string | null = null

        if (expiryDateRaw && expiryDateRaw.toString().trim() !== '') {
          let parsedDate: Date | null = null
          const dateStr = expiryDateRaw.toString().trim()
          
          // أولاً: التحقق إذا كان رقم Excel serial date
          const serialDate = parseFloat(dateStr)
          if (!isNaN(serialDate) && serialDate > 30000 && serialDate < 60000) {
            // Excel serial date: الأيام منذ 1900-01-01
            // نطرح 25569 للتحويل إلى Unix timestamp (أيام منذ 1970-01-01)
            const unixDays = serialDate - 25569
            parsedDate = new Date(unixDays * 86400 * 1000)
          } else {
          ...
          if (parsedDate && !isNaN(parsedDate.getTime()) && parsedDate.getFullYear() > 2000 && parsedDate.getFullYear() < 2100) {
