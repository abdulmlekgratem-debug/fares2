...
export default function SearchFilters({
  ...
  const handleNearbyFilter = useCallback(() => {
    if (!navigator.geolocation) {
      // عرض رسالة في console فقط، المتصفح الحديث يدعمه دائماً
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setIsLocating(false)
        onNearbyFilter?.(latitude, longitude, 10)
      },
      (error) => {
        setIsLocating(false)
        console.error('Geolocation error:', error)
        // لا نستخدم alert() - الواجهة تُظهر حالة isLocating بشكل كافٍ
      },
          ...
          <Tooltip content={showMap ? "اضغط لإخفاء الخريطة التفاعلية" : "اضغط لعرض مواقع اللوحات على الخريطة"} side="bottom">
            <Button
              onClick={() => {
                ...
                setShowMap(!showMap)
                // Prevent scroll jump when toggling map
