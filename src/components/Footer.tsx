...
export default function Footer({ id }: FooterProps) {
  return (
    <footer id={id} className="relative overflow-hidden bg-card/50 backdrop-blur-sm border-t border-border/30 mt-16">
      ...
      <div className="container mx-auto px-4 py-16 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="lg:col-span-1 text-center lg:text-right">
            ...
            {/* Social icons */}
            <div className="flex items-center justify-center lg:justify-start gap-2 mt-4 md:mt-6">
              <a
                href="https://www.facebook.com/AL.FARES.AL.DAHABI.LY"
                target="_blank"
                rel="noopener noreferrer"
                className="group w-9 h-9 md:w-11 md:h-11 rounded-lg md:rounded-xl bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center shadow-md hover:shadow-blue-500/30 hover:scale-110 transition-all duration-300"
              >
                <FacebookIcon className="w-4 h-4 md:w-5 md:h-5 text-white group-hover:scale-110 transition-transform" />
              </a>
              <a
                href="https://wa.me/218913228908?text=مرحباً، أريد الاستفسار عن اللوحات الإعلانية المتاحة"
                target="_blank"
                rel="noopener noreferrer"
                className="group w-9 h-9 md:w-11 md:h-11 rounded-lg md:rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-md hover:shadow-emerald-500/30 hover:scale-110 transition-all duration-300"
              >
                <WhatsAppIcon className="w-4 h-4 md:w-5 md:h-5 text-white group-hover:scale-110 transition-transform" />
              </a>
            </div>
          </div>
          ...
          <div className="lg:col-span-1 text-center lg:text-right">
            ...
            <div className="space-y-2 md:space-y-3">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center lg:justify-start gap-2 md:gap-3 px-3 md:px-4 py-2 md:py-3 rounded-lg md:rounded-xl bg-gradient-to-r from-emerald-500/10 to-emerald-600/10 border border-emerald-500/20 hover:border-emerald-500/40 hover:bg-emerald-500/20 transition-all group"
              >
                <WhatsAppIcon className="w-4 h-4 md:w-5 md:h-5 text-emerald-500" />
                <span className="text-emerald-600 dark:text-emerald-400 font-medium text-sm md:text-base">راسلنا على واتساب</span>
              </a>
              <a
                href="https://www.facebook.com/AL.FARES.AL.DAHABI.LY"
                target="_blank"
        ...
        <div className="border-t border-border/30 pt-8">
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-muted-foreground/70 text-xs leading-relaxed max-w-2xl">
              هذا الموقع مخصص لعرض اللوحات الإعلانية المتاحة فقط ولا يُعد عقدًا أو التزامًا قانونيًا.
              جميع الحجوزات والتعاقدات تتم عبر القنوات الرسمية المعتمدة من الجهة المالكة.
            </p>
          </div>
        </div>
      </div>
